...
foreach ($menuItems as $item) {
    ...
    if (isset($item['category'])) {
        if (!isset($categoryCodeCounts[$item['category']])) {
            $categoryCodeCounts[$item['category']] = 1;
        } else {
            $categoryCodeCounts[$item['category']]++;
        }
    }
}
sort($categories);
...
function generateItemCode($category, $name, $existingCodes) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code)) {
        ...
        foreach ($menuItems as $item) {
            if (isset($item['code']) && strtoupper($item['code']) === strtoupper($code)) {
        ...
        <form method="POST" action="">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div id="newCategoryContainer" style="display: none;">
                    ...
                    <input type="text" id="new_category" name="new_category" value="<?php echo isset($_POST['new_category']) ? htmlspecialchars($_POST['new_category']) : ''; ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                </div>
                
                <div>
                    <label for="code" class="block text-sm font-medium text-gray-700 mb-1">Item Code/ID *</label>
                    <div class="relative">
                        <input type="text" id="code" name="code" value="<?php echo isset($_POST['code']) ? htmlspecialchars($_POST['code']) : ''; ?>" placeholder="Auto-generated" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" readonly>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Unique identifier for the item (auto-generated)</p>
                </div>
                
                <div class="md:col-span-2">
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                    <textarea id="description" name="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"><?php echo isset($_POST['description']) ? htmlspecialchars($_POST['description']) : ''; ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="price" name="price" value="<?php echo isset($_POST['price']) ? htmlspecialchars($_POST['price']) : ''; ?>" min="0" step="0.01" class="w-full pl-7 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                    </div>
                </div>
                
                <div>
                    <label for="stock" class="block text-sm font-medium text-gray-700 mb-1">Stock Quantity</label>
                    <input type="number" id="stock" name="stock" value="<?php echo isset($_POST['stock']) ? htmlspecialchars($_POST['stock']) : '0'; ?>" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <p class="text-xs text-gray-500 mt-1">Number of items in stock</p>
                </div>
                
                <div class="md:col-span-2">
                    <div class="flex items-center">
                        <input type="checkbox" id="available" name="available" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" <?php echo (!isset($_POST['available']) || isset($_POST['available'])) ? 'checked' : ''; ?>>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        categorySelect.addEventListener('change', function() {
            ...
            // Generate code when category changes
            generateCode();
        });
        ...
        // Generate code when new category changes
        ...
        function generateCode(forceNew = false) {
            ...
            // Only generate if we have both name and category
            if (name && category && category !== 'new') {
                // Get category prefix (first 3 letters, uppercase)
                ...
                // If forcing new, increment the counter
                if (forceNew && codeInput.value) {
                    code = originalCode + counter;
                    counter++;
                }
                
                while (existingCodes.includes(code) || (forceNew && code === codeInput.value)) {
                    code = originalCode + counter;
                    counter++;
                }
